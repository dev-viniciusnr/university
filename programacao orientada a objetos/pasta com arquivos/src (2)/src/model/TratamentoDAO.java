...
// Partial CRUD    
    ...
    private Tratamento buildObject(ResultSet rs) {
        ...
        try {
            ...
            tratamento = new Tratamento(rs.getInt("id"),rs.getString("data_inicial"), rs.getString("data_final"), rs.getInt("id_animal"));
    ...
    public void update(Tratamento tratamento) {
        try {
            PreparedStatement stmt;
            stmt = DAO.getConnection().prepareStatement("UPDATE tratamento SET data_final=? WHERE id=?");
            stmt.setString(1, tratamento.getDataFinal());
