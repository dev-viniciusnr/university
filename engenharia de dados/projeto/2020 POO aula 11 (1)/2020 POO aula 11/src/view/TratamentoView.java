...
public class TratamentoView extends javax.swing.JPanel {
    ...
    List<Cliente> listaCliente = ClienteDAO.getInstance().getAllClientes();
    ...
    private void atualizaJcbCliente() {
        ...
        for (int i = 0; i < listaCliente.size(); i++) {
            clientes[i] = listaCliente.get(i).getNome();
        }
        jcbCliente.setModel(new javax.swing.DefaultComboBoxModel<>(clientes));
    }

    private void atualizaJcbAnimal() {
        listaAnimal = AnimalDAO.getInstance().getAllAnimais(listaCliente.get(jcbCliente.getSelectedIndex()).getId());
